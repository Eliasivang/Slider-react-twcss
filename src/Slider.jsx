...
const handleClickImg2 = ()=>{
  ...
  return (
    <>
    <h2 className='text-2xl text-center my-10'>SLIDER</h2>
        <div className='flex items-center justify-center w-3/4 mx-auto overflow-hidden h-80 rounded-2xl'>
                <div 
                 className='w-full h-full duration-500 bg-cover ' 
                 style={{backgroundImage: `url(${slides[index].images})`}}>
                </div>
                <div>
                    <BsChevronLeft onClick={prevImg}  className='absolute left-0 mx-4 text-4xl text-black cursor-pointer hover:text-violet-300 '/>
                    <BsChevronRight onClick={nextImg} className='absolute right-0 mx-4 text-4xl text-black cursor-pointer hover:text-violet-300 '/>
                </div> 
        </div>
